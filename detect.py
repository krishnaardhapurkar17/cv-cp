...
def run(
    ...
    # --- CUSTOM CODE START: For Size & Heatmap - Initialization ---
    # IMPORTANT: You must find the correct scale for your images!
    # This value is a placeholder. For Chandrayaan-2 OHRC images, it's around 0.25 meters/pixel.
    # Adjust this value based on the resolution of your lunar images.
    METERS_PER_PIXEL = 0.25

    # Initialize a blank canvas for the heatmap outside the main loop
    heatmap = None
    # --- CUSTOM CODE END ---
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # label = names[c] if hide_conf else f"{names[c]}" # ORIGINAL LINE
            ...
            if heatmap is not None and np.any(heatmap > 0): # Check if any boulders were actually detected
                ...
                # Blend the heatmap with the original image (im0 contains the detections)
                # The alpha value (0.5) controls the transparency of the heatmap
                superimposed_img = cv2.addWeighted(heatmap_colored, 0.5, im0, 0.5, 0)
